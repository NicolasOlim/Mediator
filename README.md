...
Se programamos essa tela do jeito "tradicional", fazendo as ligações de forma direta, a lógica do sistema vai ficar completamente espalhada. Pense no fluxo: quando o usuário clica em um cliente na lista, o código dessa lista precisa acessar diretamente as caixas de texto para preenchê-las e, logo em seguida, ir até o botão "Excluir" para ativá-lo. 

Da mesma forma, assim que alguém digita qualquer coisa em uma caixa de texto, ela precisa "avisar" o botão "Salvar" para que ele fique clicável. E, por fim, quando o botão "Salvar" é pressionado, ele próprio tem a obrigação de limpar os campos de texto e mandar a lista se atualizar.
